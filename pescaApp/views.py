...
from pescaApp.models import Cuenca, Pesca, Metodo
from pescaApp.serializers import MetodoSerializer, PescaSerializer, CuencaSerializer
...
def metodoApi(request, cod=0):
    if request.method == 'GET':
        ...
        metodo_data = JSONParser().parse(request)
        metodo = Metodo.objects.get(id=cod)
...
def cuencasApi(request, cod=0):
    if request.method == 'GET':
        ...
        cuenca_data = JSONParser().parse(request)
        cuenca = Cuenca.objects.get(id=cod)
...
def pescasApi(request, cod=0):
    if request.method == 'GET':
        ...
        pescas_data = JSONParser().parse(request)
        pescas = Pesca.objects.get(id=cod)
